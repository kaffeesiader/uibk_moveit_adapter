...
UibkRobotHW::UibkRobotHW() :initialized_(false) {
}

UibkRobotHW::~UibkRobotHW() {
}
/**
 ...
 */
bool UibkRobotHW::initialize(ros::NodeHandle &hw_nh) {
	...
	if (!hw_nh.hasParam("adapter_list")) {
		ROS_ERROR("UibkRobotHW: No adapter_list specified.");
		return false;
	}
	...
	if (adapter_list.getType() != XmlRpcValue::TypeArray) {
		ROS_ERROR("UibkRobotHW: adapter_list should be specified as an array");
		return false;
	}
...
bool UibkRobotHW::initialStateReceived() {
...
JointStateAdapter::JointStateAdapter(ros::NodeHandle &nh, JointStateInterface *state_itf, JointCommandInterface *cmd_itf):
	...
	if(!readonly_) {
		...
		pub_ = public_nh.advertise<motion_control_msgs::JointPositions>(cmd_topic, 1);
...
void JointStateAdapter::initialize(const sensor_msgs::JointState &initialState) {
	ROS_INFO("Initializing arm state adapter");
	...
	for (size_t i = 0; i < initialState.name.size(); ++i) {
		...
		// prepare our JointPositionMessage (register all the joint names used by this adapter)
		pos_msg_.names.push_back(name);
	}
...
void JointStateAdapter::Update() {
	...
	pos_msg_.positions.clear();

	for (size_t i = 0; i < pos_msg_.names.size(); ++i) {
		string jnt_name = pos_msg_.names[i];
		JointPtr jnt = joints_[jnt_name];
		pos_msg_.positions.push_back(jnt->targetPosition);
	}
